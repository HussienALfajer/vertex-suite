...
**Status** v1.17 · **Scope** the interface layer of the whole product
...
| **Inputs**     | TextInput · TextArea · NumberInput · MoneyInput · QuantityInput · DateInput · Select · Combobox · Checkbox · RadioGroup · Switch · SegmentedControl · SearchInput · FormatBuilder · AttachmentInput | TextInput · Select · Checkbox · Switch           |
...
**No user-facing string is a literal in code.** Every label, message and unit name resolves through `@vertex/i18n`'s terminology layer, from a catalogue: the sentences a component speaks for itself — a dialog's close button, the map's controls, the mark on a rate that is not today's — ship with `packages/ui` as `UI_CATALOGUE`, and an application merges them beneath its own, so it writes only what its screens say and may reword what the components say.
...
| `UI_CATALOGUE` against the components: every key formatted has a sentence, and every sentence is formatted | §12           | `packages/ui`                    |
...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Reason                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-23 | **v1.17** — `SyncStatus`, named in §10's State group since v1.0 and unbuilt until now, is built with `U07`'s `POS-18` and `SYN-06`: the register's persistent word on its store node. It leads with the connection in words, counts what is waiting, marks a refused operation with a badge of its own, and changes the shape of its icon with each condition. `Enter` opens the detail as a dialog — the connection explained for the person at the till, when the store node last answered and when it will next be asked, the queue in the order it must be applied, and for a refused operation why, what it holds up, the reference to quote and the two ways out, retry and escalate. A change of condition is announced politely. Its sentences ship in `UI_CATALOGUE`. `OfflineIndicator` stays in the inventory and unbuilt: the register states its connection here, and a second component saying half of the same thing waits for a screen that needs only that half                            | The status is the courier's (`@vertex/platform`), handed in as it is: the component restates its vocabulary rather than importing it, because the design system depends on no module and no host. The register that will carry it arrives in `U12`, and the sandbox's journeys drive it meanwhile through the real courier. Its journeys found the one thing a unit test had not: a control that removes or disables itself under the focus that pressed it drops that focus out of the dialog, where `Esc` no longer reaches — so escalating moves focus to the notice that now says so, and retry is never disabled                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-09-21 | **v1.16** — Corrections from an audit of the built layer before `U07`, none of them a new rule. §12's sentence about where a string lives becomes true of the design system's own: `packages/ui` ships the sentences its components speak as `UI_CATALOGUE`, a host merges them beneath its own catalogue, and §13 gains the test that holds the catalogue to the components both ways. `@vertex/ui/map` publishes the two components, the configurable basemap, the one search and their types, and no longer the projection, tiles, gestures and atlas behind them. `generateAccents` no longer takes the neutral ramp it never read.                                                                                                                                                                                                                                                                                                                                                                     | Every host carried its own copy of the forty-odd sentences the components speak, and had to know from reading the components which keys they would ask for. The two hosts had drifted: the back office lacked `rate.notToday` and the provisional-date pair, and would have thrown on the first rate board that showed yesterday's rate. A sentence that ships with the component that speaks it cannot be missing from a host, and a test over the components cannot be argued with. The map's entry point had published its whole machinery and nothing outside the package ever named any of it; an entry point grows with the screen that needs it (§15), and this one had grown ahead of any.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-09-21 | **v1.15** — §10 gains `AttachmentInput` in Inputs, making the inventory **28**, and `MoneyInput` and `Combobox` — named there since v1.0 and unbuilt until now — are built. All three arrive with `FIN-02`–`FIN-07`'s ledger screens: the journal, the accountant's own entry, the opening balances, the exceptions queue and the four statements. `MoneyInput` speaks the kernel's `Money` and takes only a keystroke that leaves an amount its currency can hold. `Combobox` is a closed list narrowed by typing, searching an option's detail as well as its label and folding what it compares. `AttachmentInput` hands over bytes rather than `File` handles, turns away what the caller would refuse and says which and why. The status line, which had stayed at v1.13 while v1.14 shipped, is moved with this entry                                                                                                                                                                                 | A number field cannot be used for money at all: every one of them in every library speaks `number`, and a JavaScript float cannot represent `0.1` — the error, once made, is only ever carried forward (`README.md`). A `Select` over a chart of three hundred accounts technically offers the account and practically hides it, which is the line between the two controls rather than a preference about their size. And an attachment is evidence for a posting (`FIN-04`): a `File` is a live handle onto a disk that can answer differently on a second read and cannot be held in the state of a form still being filled in, so what crosses the boundary is the bytes whose hash goes onto the entry                                                                                                                                                                                                                                                                                                                                                                                                                         |
