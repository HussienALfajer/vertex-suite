...
| `<DateTime>`     | Renders in the branch's timezone with the zone stated — beside a date-only value unless the caller's own context already names whose day it is (`showZone`). A **provisional business date** (a shift opened without the store node) is marked visibly wherever it appears (`POS-01`) |
